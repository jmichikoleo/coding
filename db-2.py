...
def main():
    ...
    data = pd.read_csv('dfcombined.csv')
    data['Transaction'] = pd.to_datetime(data['Transaction'])
    products = data['Product'].unique()
    selected_product = st.selectbox('Select Product', products)
    filtered_df = data[data['Product'] == selected_product]
        
    st.write(f"### Sales Time Series for {selected_product}")
    fig, ax = plt.subplots(figsize=(10, 6))
    filtered_df.set_index('Transaction')['Product name'].resample('D').count().plot(ax=ax)
    plt.xlabel('Date')
